package com.callx.app.utils;

import android.content.Context;
import android.content.SharedPreferences;
import com.google.firebase.database.*;
import java.util.*;

/** StatusCloseFriendsManager v26 — FIX: real-time ValueEventListener for multi-device sync. */
public final class StatusCloseFriendsManager {
    private static final String PREFS = "close_friends_prefs";
    private static final String KEY   = "cf_uids";
    private static ValueEventListener liveListener;
    private StatusCloseFriendsManager() {}

    public static boolean isCloseFriend(Context ctx, String uid) {
        return uid != null && getLocalList(ctx).contains(uid);
    }
    public static Set<String> getLocalList(Context ctx) {
        return new HashSet<>(prefs(ctx).getStringSet(KEY, new HashSet<>()));
    }
    public static void addCloseFriend(Context ctx, String myUid, String uid) {
        if (uid == null || myUid == null) return;
        Set<String> s = getLocalList(ctx); s.add(uid); save(ctx, s);
        getRef(myUid).child(uid).setValue(true);
    }
    public static void removeCloseFriend(Context ctx, String myUid, String uid) {
        if (uid == null || myUid == null) return;
        Set<String> s = getLocalList(ctx); s.remove(uid); save(ctx, s);
        getRef(myUid).child(uid).removeValue();
    }
    public static void toggle(Context ctx, String myUid, String uid) {
        if (isCloseFriend(ctx, uid)) removeCloseFriend(ctx, myUid, uid);
        else                          addCloseFriend(ctx, myUid, uid);
    }
    /** FIX: Real-time sync — multi-device changes reflect instantly */
    public static void startRealtimeSync(Context ctx, String myUid) {
        if (myUid == null) return;
        DatabaseReference ref = getRef(myUid);
        if (liveListener != null) ref.removeEventListener(liveListener);
        liveListener = new ValueEventListener() {
            @Override public void onDataChange(DataSnapshot snap) {
                Set<String> s = new HashSet<>();
                for (DataSnapshot c : snap.getChildren()) if (c.getKey() != null) s.add(c.getKey());
                save(ctx, s);
            }
            @Override public void onCancelled(DatabaseError e) {}
        };
        ref.addValueEventListener(liveListener);
    }
    public static void stopRealtimeSync(String myUid) {
        if (liveListener != null && myUid != null) {
            getRef(myUid).removeEventListener(liveListener); liveListener = null;
        }
    }
    private static DatabaseReference getRef(String myUid) {
        return FirebaseUtils.db().getReference("closeFriends").child(myUid);
    }
    private static void save(Context ctx, Set<String> s) { prefs(ctx).edit().putStringSet(KEY, s).apply(); }
    private static SharedPreferences prefs(Context ctx) { return ctx.getSharedPreferences(PREFS, Context.MODE_PRIVATE); }
}
