// callx-server v4 — Production-grade Node.js / Express backend
// ─────────────────────────────────────────────────────────────────────────────
// Upgraded: /notify/status endpoint now sends rich FCM payloads including
//   sender photo, statusType, mediaUrl, and text so the Android app can build
//   BigPicture notifications even when the app process was killed.
//
// New endpoints:
//   POST /notify/status/seen   — fan-out seen-receipt notifications to status owner
//   POST /status/cleanup       — server-side TTL cleanup (call via Cloud Scheduler)
// ─────────────────────────────────────────────────────────────────────────────
...
// ── Status notify — UPGRADED PRODUCTION VERSION ───────────────────────────
//
// Now fans out to all contacts with a rich FCM data payload:
//   type        = "status"
//   fromUid     = poster UID
//   fromName    = poster display name
//   fromPhoto   = poster avatar URL (for large-icon in notification)
//   statusType  = "text" | "image" | "video"
//   text        = status text or caption
//   mediaUrl    = image/video URL (for BigPicture style notification)
//
// The Android StatusBackgroundService and CallxMessagingService consume this
// payload to build rich, media-preview notifications even when the app is killed.
...
app.post("/notify/status", statusLimiter, async (req, res) => {
  ...
  try {
    // Fetch poster's photo if not supplied
    ...
    // Get all contacts of the status poster
...
// ── Status seen fan-out (notify owner when someone views their status) ────
//
// Called by the Android client after exiting StatusViewerActivity.
// Sends an FCM data message to the status owner so they can refresh seen counts.
// Uses "normal" priority — this is non-urgent.
...
// ── Status TTL cleanup (call via Cloud Scheduler every hour) ─────────────
//
// Scans all status/{uid}/{statusId} nodes and removes entries older than 24h.
// This keeps the DB clean and reduces bandwidth for connected clients.

app.post("/status/cleanup", async (req, res) => {
  if (!firebaseReady) return res.status(503).json({ error: "Firebase not configured" });
  // Simple auth: require a server-side secret header
...
app.listen(PORT, () => console.log(`callx-server v4 on :${PORT}`));
