...
//
// Android client calls POST /cloudinary/sign with:
//   { "folder": "callx/videos/file", "resource_type": "video" }
// Server responds with signed params so client can upload directly to Cloudinary.
// This keeps API secret off the device.
...
const cloudinaryLimiter = rateLimit({ windowMs: 60_000, max: 60 });

app.post("/cloudinary/sign", cloudinaryLimiter, (req, res) => {
  res.set("Cache-Control", "no-store");
  ...
  const { folder = "callx", resource_type = "auto" } = req.body || {};
  const timestamp = String(Math.round(Date.now() / 1000));

  // Params to sign (must be sorted alphabetically, no file/api_key)
