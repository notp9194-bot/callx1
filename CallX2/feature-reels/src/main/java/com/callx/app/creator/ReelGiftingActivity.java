...
public class ReelGiftingActivity extends AppCompatActivity {
    ...
    static class TopAdp extends RecyclerView.Adapter<TopAdp.VH>{
        ...
        @Override public void onBindViewHolder(@NonNull VH h,int pos){TopG g=items.get(pos);h.tvRank.setText("#"+g.rank);h.tvName.setText(g.name.isEmpty()?"User":g.name);h.tvCoins.setText(fmt(g.coins)+" coins");if(!g.photo.isEmpty()) Glide.with(h.iv).load(g.photo).circleCrop().placeholder(R.drawable.ic_person).override(96, 96).into(h.iv);com.callx.app.utils.StoryRingApplier.applyWithClick(h.itemView.getContext(),h.ivStoryRing,g.uid);}
        @Override public int getItemCount(){return items.size();}
        private static String fmt(long n){if(n>=1000) return String.format("%.1fK",n/1000.0);return String.valueOf(n);}
        static class VH extends RecyclerView.ViewHolder{CircleImageView iv;ImageView ivStoryRing;TextView tvRank,tvName,tvCoins;VH(View v){super(v);iv=v.findViewById(R.id.iv_gifter_avatar);ivStoryRing=v.findViewById(R.id.iv_story_ring);tvRank=v.findViewById(R.id.tv_gifter_rank);tvName=v.findViewById(R.id.tv_gifter_name);tvCoins=v.findViewById(R.id.tv_gifter_coins);}}
    }
}
