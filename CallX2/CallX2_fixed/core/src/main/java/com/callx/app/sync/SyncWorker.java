    ...
    public Result doWork() {
        ...
        try {
            ...
            if (myUidSync != null) {
                ...
                List<com.callx.app.db.entity.MessageEntity> pendingMsgs =
                        db.messageDao().getAllPendingMessages();
                if (pendingMsgs != null) {
                    for (com.callx.app.db.entity.MessageEntity pe : pendingMsgs) {
                        // Sirf text messages retry karo yahan (media alag step mein)
                        if (pe.mediaLocalPath != null && !pe.mediaLocalPath.isEmpty()) continue;
                        boolean isGroup = Boolean.TRUE.equals(pe.isGroup);
                        com.google.firebase.database.DatabaseReference msgRef = isGroup
                            ? FirebaseUtils.getGroupMessagesRef(pe.chatId).child(pe.id)
                            : FirebaseUtils.getMessagesRef(pe.chatId).child(pe.id);
                        java.util.Map<String, Object> m = new java.util.HashMap<>();
                        m.put("id",         pe.id);
                        m.put("senderId",   pe.senderId);
                        m.put("senderName", pe.senderName);
                        m.put("text",       pe.text);
                        m.put("type",       pe.type != null ? pe.type : "text");
                        m.put("timestamp",  pe.timestamp);
                        final String finalMsgId = pe.id;
                        msgRef.setValue(m)
                            .addOnSuccessListener(u ->
                                new Thread(() -> db.messageDao().updateStatus(finalMsgId, "sent")).start());
                    }
                    Log.d(TAG, "v18: Pending messages retry — " + pendingMsgs.size() + " msgs");
                }

                // IMPROVEMENT 4: Pending read receipts flush
                List<ChatEntity> pendingReads = db.chatDao().getPendingMarkReadChats();
                if (pendingReads != null) {
                    for (ChatEntity chat : pendingReads) {
                        // Private chat read receipt
                        if ("private".equals(chat.type) && chat.partnerUid != null) {
                            FirebaseUtils.getContactsRef(myUidSync)
                                .child(chat.partnerUid).child("unread").setValue(0)
                                .addOnSuccessListener(u -> {
                                    new Thread(() -> db.chatDao().clearPendingMarkRead(chat.chatId)).start();
                                });
                        } else {
                            // Group unread reset
                            FirebaseUtils.db().getReference("groups")
                                .child(chat.chatId).child("unread").child(myUidSync).setValue(0)
                                .addOnSuccessListener(u -> {
                                    new Thread(() -> db.chatDao().clearPendingMarkRead(chat.chatId)).start();
                                });
                        }
                    }
                    Log.d(TAG, "v18: Pending read receipts flush — " + pendingReads.size() + " chats");
                }
            }
            ...
            if (isHeavy) {
                ...
                if (autoDeleteMs > 0) {
                    ...
                    // Step A: Room se purane messages delete karo
                    int deletedCount = db.messageDao().deleteMessagesOlderThan(cutoff);
                    Log.d(TAG, "Auto-delete: removed " + deletedCount
                            + " messages older than " + secMgr.getAutoDeleteLabel());
                    ...
                    if (myUid != null && chats != null) {
                        for (ChatEntity chat : chats) {
                            ...
                            // sirf admin/server delete kar sakta hai
                            if (!"private".equals(chat.type)) continue;
                            FirebaseUtils.getMessagesRef(chat.chatId)
                                ...
                                .addListenerForSingleValueEvent(
                                    new com.google.firebase.database.ValueEventListener() {
                                        ...
                                        public void onDataChange(
                                            ...
                                            for (com.google.firebase.database.DataSnapshot c
                                                    : snap.getChildren()) {
                        ...
                        Log.d(TAG, "Auto-delete: Firebase cleanup triggered for "
                                + chats.size() + " chats");
                    }
                }
                ...
                List<com.callx.app.db.entity.MessageEntity> failedUploads =
                        db.messageDao().getFailedMediaUploads();
                if (failedUploads != null && !failedUploads.isEmpty()) {
                    Log.d(TAG, "v18: Retrying " + failedUploads.size() + " failed media uploads");
                    for (com.callx.app.db.entity.MessageEntity fe : failedUploads) {
                        android.net.Uri localUri = android.net.Uri.parse(fe.mediaLocalPath);
                        String folder   = Boolean.TRUE.equals(fe.isGroup) ? "callx/group_media" : "callx/media";
                        String resType  = fe.mediaResourceType != null ? fe.mediaResourceType : "auto";
                        final String fId = fe.id;
                        final String fChatId = fe.chatId;
                        final boolean fIsGroup = Boolean.TRUE.equals(fe.isGroup);
                        com.callx.app.utils.CloudinaryUploader.upload(
                            getApplicationContext(), localUri, folder, resType,
                            new com.callx.app.utils.CloudinaryUploader.UploadCallback() {
                                @Override
                                public void onSuccess(com.callx.app.utils.CloudinaryUploader.Result r) {
                                    // Upload success — mediaUrl update karo + Firebase push karo
                                    new Thread(() -> {
                                        com.callx.app.db.entity.MessageEntity updated =
                                            db.messageDao().getMessageById(fId);
                                        ...
                                        // Firebase push
                                        com.google.firebase.database.DatabaseReference ref = fIsGroup
                                            ? FirebaseUtils.getGroupMessagesRef(fChatId).child(fId)
                                            : FirebaseUtils.getMessagesRef(fChatId).child(fId);
                                        java.util.Map<String, Object> upd = new java.util.HashMap<>();
                                        upd.put("mediaUrl",     r.secureUrl);
                                        upd.put("thumbnailUrl", r.thumbnailUrl != null ? r.thumbnailUrl : "");
                                        upd.put("status",       "sent");
                                        ref.updateChildren(upd);
                                        Log.d(TAG, "v18: Media retry success for " + fId);
                                    }).start();
                                }
                                @Override
                                public void onError(String msg) {
                                    Log.w(TAG, "v18: Media retry failed for " + fId + ": " + msg);
                                    // pending rakho — agli HEAVY sync mein retry
                                }
                            });
                    }
                }

                // ── v18 IMPROVEMENT 6: Unread badge Room se sync ─────────
                // Firebase offline hone par badge wrong/0 ho jata tha.
                // Room.ChatEntity.unread already correct hai — kuch extra nahi karna,
                // ChatsFragment LiveData se automatically Room ka badge read karta hai.
                // Yeh comment ensures developers understand intent.
                Log.d(TAG, "v18: Unread badges Room-sourced — no extra sync needed");
