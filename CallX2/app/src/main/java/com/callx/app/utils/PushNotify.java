...
public class PushNotify {
    ...
    /**
     * Notify all group members about an incoming group call.
     * The server must fan-out to all group member FCM tokens.
     * FCM payload type: "group_call" — triggers GroupCallRingService in CallxMessagingService.
     */
    public static void notifyGroupCall(String groupId, String callerUid, String callerName,
                                       String callId, boolean isVideo) {
        try {
            JSONObject body = new JSONObject()
                .put("groupId",     groupId    == null ? "" : groupId)
                .put("fromUid",     callerUid  == null ? "" : callerUid)
                .put("fromName",    callerName == null ? "" : callerName)
                .put("type",        Constants.GCALL_FCM_TYPE)
                .put("callId",      callId     == null ? "" : callId)
                .put("isVideo",     isVideo)
                .put("text",        (isVideo ? "Group video call" : "Group voice call")
                                    + " started by " + (callerName == null ? "Someone" : callerName));
            Request req = new Request.Builder()
                .url(Constants.SERVER_URL + "/notify/group/call")
                .post(RequestBody.create(body.toString(),
                    MediaType.parse("application/json")))
                .build();
            client.newCall(req).enqueue(new Callback() {
                @Override public void onFailure(Call call, java.io.IOException e) {
                    Log.w("PushNotify", "gcall fail: " + e.getMessage());
                }
                @Override public void onResponse(Call call, Response response) {
                    if (response.body() != null) response.close();
                }
            ...
            Log.w("PushNotify", "gcall err: " + e.getMessage());
        }
    }

    /**
     * Notify a missed group call to participants who did not join.
     */
    public static void notifyMissedGroupCall(String groupId, String callerUid,
                                              String callerName, String callId,
                                              boolean isVideo) {
        try {
            JSONObject body = new JSONObject()
                .put("groupId",   groupId    == null ? "" : groupId)
                .put("fromUid",   callerUid  == null ? "" : callerUid)
                .put("fromName",  callerName == null ? "" : callerName)
                .put("type",      "group_call_missed")
                .put("callId",    callId     == null ? "" : callId)
                .put("isVideo",   isVideo)
                .put("text",      "Missed group " + (isVideo ? "video" : "voice") + " call");
            Request req = new Request.Builder()
                .url(Constants.SERVER_URL + "/notify/group/call/missed")
                .post(RequestBody.create(body.toString(),
                    MediaType.parse("application/json")))
                .build();
            client.newCall(req).enqueue(new Callback() {
