...
public abstract class AppDatabase extends RoomDatabase {
    ...
    private static AppDatabase buildDatabase(Context ctx) {
        // Try encrypted DB first (SQLCipher). If it fails on this device/emulator,
        // fall back to plain Room so the app stays functional.
        try {
            ...
            if (passphrase != null && passphrase.length > 0) {
                ...
                AppDatabase db = Room.databaseBuilder(ctx, AppDatabase.class, DB_NAME)
                        ...
                        .build();
                Log.d(TAG, "AppDatabase (SQLCipher encrypted) ready");
                return db;
            }
        } catch (Exception e) {
            Log.w(TAG, "SQLCipher init failed, falling back to plain DB: " + e.getMessage());
        }

        // Fallback: plain (unencrypted) Room DB — safe for dev/emulator use
        AppDatabase db = Room.databaseBuilder(ctx, AppDatabase.class, DB_NAME + ".plain")
                .fallbackToDestructiveMigration()
                .build();
        Log.w(TAG, "AppDatabase (plain, unencrypted fallback) ready");
        return db;
    }
}
