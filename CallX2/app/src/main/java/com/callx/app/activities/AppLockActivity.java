...
public class AppLockActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Fix #11: Setup screen pe bhi screenshot block karo
        getWindow().addFlags(WindowManager.LayoutParams.FLAG_SECURE);
        binding = ActivityAppLockBinding.inflate(getLayoutInflater());
        setContentView(binding.getRoot());
        binding.toolbar.setNavigationOnClickListener(v -> finish());

        // FIX-AL1: Singleton use karo — heavy EncryptedSharedPreferences re-init avoid
        ...
        BiometricPrompt prompt = new BiometricPrompt(this, executor,
            new BiometricPrompt.AuthenticationCallback() {
                ...
                @Override public void onAuthenticationError(int code, CharSequence msg) {
                    // User cancel ya error — silently ignore (setup screen pe)
                    Toast.makeText(AppLockActivity.this,
                        "Auth cancelled", Toast.LENGTH_SHORT).show();
                }
                @Override public void onAuthenticationFailed() {
                    // FIX-AL: Record failure for lockout tracking
                    lockManager.recordBiometricFailure();
                    if (lockManager.isLockedOut()) {
                        Toast.makeText(AppLockActivity.this,
                            "Too many attempts. Try again later.", Toast.LENGTH_LONG).show();
                    } else {
                        Toast.makeText(AppLockActivity.this,
                            "Authentication failed (" + lockManager.getRemainingAttempts() + " attempts left)",
                            Toast.LENGTH_SHORT).show();
                    }
                }
            });

        // FIX-LS2 related: AppLockActivity setup/unlock prompt:
        // Agar DEVICE_CREDENTIAL allowed hai toh NegativeButtonText nahi daal sakte.
        // Setup mode (isSetup=true) mein NegativeButton safe hai (STRONG only).
        // Unlock mode (isSetup=false) mein STRONG | DEVICE_CREDENTIAL (no NegativeButton).
        BiometricPrompt.PromptInfo info;
        if (isSetup) {
            // Setup — BIOMETRIC_STRONG only, NegativeButton safe
            info = new BiometricPrompt.PromptInfo.Builder()
                .setTitle("Biometric Confirm Karo")
                .setSubtitle("Apna fingerprint ya face scan karo")
                .setNegativeButtonText("Cancel")
                .setAllowedAuthenticators(BiometricManager.Authenticators.BIOMETRIC_STRONG)
                .build();
        } else {
            // Unlock — STRONG + DEVICE_CREDENTIAL, NO NegativeButtonText
            info = new BiometricPrompt.PromptInfo.Builder()
