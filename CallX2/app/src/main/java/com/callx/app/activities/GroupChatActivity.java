...
public class GroupChatActivity extends AppCompatActivity
    ...
    private void pushMessage(Message msg) {
        ...
        PushNotify.notifyGroupRich(groupId, currentUid, currentName, "", msg.id, msg.type != null ? msg.type : "group_message", previewText(msg), msg.mediaUrl != null ? msg.mediaUrl : "");
    ...
    private void toggleRecording() {
        if (!isRecording) {
            ...
            android.net.Uri audioUri = recorder.stop(this); isRecording=false; binding.btnMic.setImageResource(R.drawable.ic_mic);
            if (audioUri != null) uploadAndSend(audioUri, "audio");
        }
    }

    private void uploadAndSend(Uri uri, String type) {
        Toast.makeText(this,"Uploading…",Toast.LENGTH_SHORT).show();
        String resourceType = ("image".equals(type)||"gif".equals(type)) ? "image" : "video".equals(type) ? "video" : "raw";
        CloudinaryUploader.upload(this, uri, type, resourceType, new CloudinaryUploader.UploadCallback() {
            @Override public void onSuccess(CloudinaryUploader.Result result) {
                String url = result.secureUrl; String fn = result.publicId; long size = result.bytes != null ? result.bytes : 0L;
                Message msg = buildBaseMessage();
                msg.type=type; msg.mediaUrl=url; msg.thumbnailUrl=null; msg.fileName=fn; msg.fileSize=size;
            ...
            @Override public void onError(String err) { Toast.makeText(GroupChatActivity.this,"Upload failed",Toast.LENGTH_SHORT).show(); }
        });
    }
        ...
        FirebaseUtils.getGroupsRef().child(groupId).child("pinnedMsg")
                .addValueEventListener(new ValueEventListener() {
                    @Override public void onDataChange(DataSnapshot s) {
                        ...
                        if (has&&binding.tvPinnedPreview!=null) binding.tvPinnedPreview.setText(s.child("text").getValue(String.class));
                    }
                    @Override public void onCancelled(DatabaseError e) {}
                });
    }
    ...
    private void showReplyBar(Message m) {
        ...
        if (binding.tvReplyBarText!=null) binding.tvReplyBarText.setText(m.text!=null?m.text:previewText(m));
    }
