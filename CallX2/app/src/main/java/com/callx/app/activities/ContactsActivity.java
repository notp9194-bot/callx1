...
public class ContactsActivity extends AppCompatActivity {
    ...
    private static class ContactsAdapter
        ...
        static class VH extends RecyclerView.ViewHolder {
            ...
            TextView  tvName;
            CheckBox  cbSelect;
            // Ring dedupe guard (mirrors HomeFragment post-row / ChatActivity
            // header pattern) — RecyclerView rows get rebound on every
            // scroll/notify, so without this every bind would re-run
            // StatusCacheManager lookups + setBackground/setVisibility even
            // when nothing changed for that row.
            String lastRingKey;
            int lastRingState = -1;
            VH(@NonNull View v) {
                super(v);
                ivAvatar     = v.findViewById(R.id.iv_avatar);
                ivAvatarRing = v.findViewById(R.id.iv_avatar_ring);
