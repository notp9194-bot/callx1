...
import com.callx.app.activities.CacheStatsActivity;
...
public class AccountMenuActivity extends AppCompatActivity {
    ...
    private void setupMenuRows() {
        ...
        // Row: Edit Profile
        configureRow(binding.rowProfile.getRoot(), R.drawable.ic_person,
            "Edit Profile", "Name, photo, about");
        binding.rowProfile.getRoot().setOnClickListener(v ->
            startActivity(new Intent(this, ProfileActivity.class)));

        // Row: CallX ID
        configureRow(binding.rowCallxId.getRoot(), R.drawable.ic_person_add,
            "My CallX ID", myCallxId.isEmpty() ? "Tap to copy" : myCallxId);
        binding.rowCallxId.getRoot().setOnClickListener(v -> {
            if (myCallxId.isEmpty()) return;
        ...
        configureRow(binding.rowPrivacy.getRoot(), R.drawable.ic_phone,
            "Privacy & Security", "App lock, fingerprint, PIN, pattern");
        binding.rowPrivacy.getRoot().setOnClickListener(v ->
            startActivity(new Intent(this, PrivacySecurityActivity.class)));

        // Row: Notifications
        configureRow(binding.rowNotifications.getRoot(), R.drawable.ic_status_notification,
            "Notifications", "Message and call alerts");
        binding.rowNotifications.getRoot().setOnClickListener(v ->
            Toast.makeText(this, "Notification settings — coming soon", Toast.LENGTH_SHORT).show());

        // Row: Chats
        configureRow(binding.rowChats.getRoot(), R.drawable.ic_message_notification,
        ...
        // Row: Storage & Data → Cache Stats Activity
        configureRow(binding.rowStorage.getRoot(), R.drawable.ic_file,
            "Storage & Cache", "Cache size, hit rate, encrypted DB, clear cache");
        binding.rowStorage.getRoot().setOnClickListener(v ->
            startActivity(new Intent(this, CacheStatsActivity.class)));

        // Row: Help
        configureRow(binding.rowHelp.getRoot(), R.drawable.ic_search,
        ...
        // Row: About
        configureRow(binding.rowAbout.getRoot(), R.drawable.ic_group,
            "About CallX", "App version and licenses");
        binding.rowAbout.getRoot().setOnClickListener(v -> showAboutDialog());

        // Row: Logout
        configureRow(binding.rowLogout.getRoot(), R.drawable.ic_logout,
            "Logout", null);
