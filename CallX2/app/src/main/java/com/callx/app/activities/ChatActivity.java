...
public class ChatActivity extends AppCompatActivity
    ...
    // Partner avatar URL (for zoom-open on tap)
    private String partnerPhotoUrl = null;
    ...
    private void setupToolbar() {
        ...
        // Toolbar title/name area click → open partner profile
        binding.toolbar.setOnClickListener(v -> openPartnerProfile());

        loadPartnerAvatar();

        // Avatar click (left side) → open partner profile
    ...
    private void openPartnerProfile() {
        Intent pi = new Intent(ChatActivity.this, ProfileActivity.class);
        pi.putExtra("uid",  partnerUid);
        pi.putExtra("name", partnerName);
        startActivity(pi);
    }
            ...
            .addListenerForSingleValueEvent(new ValueEventListener() {
                @Override public void onDataChange(DataSnapshot snap) {
                    ...
                    if (photo != null && !photo.isEmpty()) {
                        partnerPhotoUrl = photo; // store for zoom
    ...
    /** Full-screen avatar zoom dialog. */
    ...
    @Override public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.action_view_profile) { // N3 — zoom avatar
