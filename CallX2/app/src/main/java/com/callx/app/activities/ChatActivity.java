...
public class ChatActivity extends AppCompatActivity
    ...
    private void setupToolbar() {
        ...
        // Clicking the toolbar title area opens partner profile
        binding.toolbar.setOnClickListener(v -> openPartnerProfile());

        loadPartnerAvatar();

        // Avatar click → open partner profile (avatar is on the right side)
        if (binding.ivPartnerAvatar != null) {
            binding.ivPartnerAvatar.setOnClickListener(v -> openPartnerProfile());
        }
    ...
    @Override public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.action_view_profile) { // N3
