...
public class CallsFragment extends Fragment implements CallHistoryAdapter.SelectionListener {
    ...
    private final List<String> onlineContactUids = new ArrayList<>();
    ...
    private void renderOnlineUsers(List<User> users) {
        ...
        for (User u : users) {
            ...
            final User finalU = u;
            item.setOnClickListener(v -> openChat(finalU));
            llOnlineUsers.addView(item);
        }
    }

    private void showNoOnline() {
        if (llOnlineUsers == null || tvNoOnline == null) return;
        llOnlineUsers.setVisibility(View.GONE);
    ...
    private void deleteSelected() {
        ...
        for (CallLog log : selected) {
            if (log.id != null)
                FirebaseUtils.getCallsRef(uid).child(log.id).removeValue();
        }
