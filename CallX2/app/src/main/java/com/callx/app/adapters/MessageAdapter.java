...
public class MessageAdapter extends RecyclerView.Adapter<MessageAdapter.VH> {
    ...
    // Cache for sender photo URLs (uid -> photoUrl)
    ...
    @Override public void onBindViewHolder(@NonNull VH h, int pos) {
        ...
        // Group sender avatar (received only, left side of message)
        if (h.ivSenderAvatar != null) {
            ...
            if (showAvatar) {
                ...
                final String senderUid  = m.senderId;
                final String senderName = m.senderName;
                h.ivSenderAvatar.setOnClickListener(v -> {
                    if (senderUid == null) return;
                    Intent i = new Intent(ctx, ChatActivity.class);
                    i.putExtra("partnerUid",  senderUid);
                    i.putExtra("partnerName", senderName != null ? senderName : "");
                    ctx.startActivity(i);
                });
            } else {
    ...
    // Load sender avatar from Firebase (cached)
    private void loadSenderAvatar(Context ctx, String uid, CircleImageView iv) {
        if (uid == null) return;
        if (photoCache.containsKey(uid)) {
            String url = photoCache.get(uid);
        ...
        if (h.tvStatus != null) {
            if (sent) {
                ...
                switch (m.status == null ? "sent" : m.status) {
                    case "read":
                        h.tvStatus.setText("\u2713\u2713");
                        h.tvStatus.setTextColor(0xFF40C4FF); break;
                    case "delivered":
                        h.tvStatus.setText("\u2713\u2713");
                        h.tvStatus.setTextColor(0xAAFFFFFF); break;
                    default:
                        h.tvStatus.setText("\u2713");
                        h.tvStatus.setTextColor(0xAAFFFFFF); break;
                }
    ...
    private void showActionSheet(Context ctx, Message m, boolean sent) {
        ...
        for (int i = 0; i < emojiIds.length; i++) {
            ...
            if (et != null) {
                ...
                et.setOnClickListener(v -> { sheet.dismiss(); if (actionListener != null) actionListener.onReact(m, emoji); });
            }
        }

        sv.findViewById(R.id.action_reply).setOnClickListener(v -> { sheet.dismiss(); if (actionListener != null) actionListener.onReply(m); });

        TextView editBtn = sv.findViewById(R.id.action_edit);
        if (sent && "text".equals(m.type) && !Boolean.TRUE.equals(m.deleted)) {
            editBtn.setVisibility(View.VISIBLE);
            editBtn.setOnClickListener(v -> { sheet.dismiss(); if (actionListener != null) actionListener.onEdit(m); });
        }
        ...
        if (copyBtn != null) {
            ...
            if (isText && hasText && !Boolean.TRUE.equals(m.deleted)) {
                ...
                copyBtn.setOnClickListener(v -> { sheet.dismiss(); if (actionListener != null) actionListener.onCopy(m); });
            } else { copyBtn.setVisibility(View.GONE); }
        }

        if (!Boolean.TRUE.equals(m.deleted))
            sv.findViewById(R.id.action_forward).setOnClickListener(v -> { sheet.dismiss(); if (actionListener != null) actionListener.onForward(m); });
        else
        ...
        starBtn.setOnClickListener(v -> { sheet.dismiss(); if (actionListener != null) actionListener.onStar(m); });

        TextView pinBtn = sv.findViewById(R.id.action_pin);
        pinBtn.setText(Boolean.TRUE.equals(m.pinned) ? "\uD83D\uDCCC  Unpin" : "\uD83D\uDCCC  Pin Message");
        pinBtn.setOnClickListener(v -> { sheet.dismiss(); if (actionListener != null) actionListener.onPin(m); });

        TextView infoBtn = sv.findViewById(R.id.action_info);
        if (infoBtn != null) {
            if (sent) {
                infoBtn.setVisibility(View.VISIBLE);
                infoBtn.setOnClickListener(v -> { sheet.dismiss(); if (actionListener != null) actionListener.onInfo(m); });
        ...
        if (!Boolean.TRUE.equals(m.deleted)) {
            ...
            deleteBtn.setOnClickListener(v -> { sheet.dismiss(); if (actionListener != null) actionListener.onDelete(m); });
        }
    ...
    public void releasePlayer() {
        if (player != null) { try { player.release(); } catch (Exception ignored) {} player = null; playingPos = -1; }
    }
