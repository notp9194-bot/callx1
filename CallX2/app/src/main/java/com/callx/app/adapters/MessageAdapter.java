        ...
        switch (type) {
            ...
            case "video": {
                if (h.flVideo != null) {
                    ...
                    android.util.Log.d("VideoLoad", "Loading video thumb: " + thumb + " | main: " + url);
                    ...
                    if (cachedThumb != null) {
                        ...
                        MediaCache.get(ctx, thumb, new MediaCache.Callback() {
                            @Override public void onReady(java.io.File file) {
                                android.util.Log.d("VideoLoad", "Thumb cached: " + file.getAbsolutePath());
                            }
                            @Override public void onError(String reason) {}
                        });
                    }
                    
                    // Pre-cache main video in background
                    java.io.File cachedVideo = MediaCache.getCached(ctx, url);
                    if (cachedVideo == null && url != null && !url.isEmpty()) {
                        android.util.Log.d("VideoLoad", "Pre-caching video in background: " + url);
                        MediaCache.get(ctx, url, new MediaCache.Callback() {
                            @Override public void onReady(java.io.File file) {
                                android.util.Log.d("VideoLoad", "Video cached: " + file.getAbsolutePath());
                            }
                            @Override public void onError(String reason) {
                                android.util.Log.w("VideoLoad", "Failed to cache video: " + reason);
                            }
                        });
                    }
