...
public class MessageAdapter extends RecyclerView.Adapter<MessageAdapter.VH> {
    ...
    // Per-user bubble color presets (5 colors for received messages)
    ...
    private static final int[] BUBBLE_COLORS_DARK = {
        0xFF6D28D9, 0xFF1D4ED8, 0xFF0F766E, 0xFFC2410C, 0xFFBE185D
    };
    private final java.util.Map<String,Integer> userColorMap = new java.util.HashMap<>();

    private int getBubbleColorForUser(String uid) {
        if (uid == null) return BUBBLE_COLORS[0];
        if (!userColorMap.containsKey(uid)) {
            int idx = Math.abs(uid.hashCode()) % BUBBLE_COLORS.length;
            userColorMap.put(uid, idx);
        }
        int idx = userColorMap.get(uid);
        return BUBBLE_COLORS[idx];
    }

    // Cache for sender avatars
    ...
    @Override public void onBindViewHolder(@NonNull VH h, int pos) {
        ...
        if (h.ivSenderAvatar != null) {
            ...
            if (showAvatar) loadSenderAvatar(ctx, m.senderId, h.ivSenderAvatar);
        }
            ...
            if (sent) {
                ...
                switch (m.status == null ? "sent" : m.status) {
                    case "read":
                        ...
                        h.tvStatus.setTypeface(null, android.graphics.Typeface.BOLD);
                        h.tvStatus.setTextColor(0xFF1E90FF); break;
                    case "delivered":
                        h.tvStatus.setText("\u2713\u2713");
                        h.tvStatus.setTextSize(13f);
                        h.tvStatus.setTypeface(null, android.graphics.Typeface.BOLD);
                        h.tvStatus.setTextColor(0xDDFFFFFF); break;
                    default:
                        h.tvStatus.setText("\u2713");
                        h.tvStatus.setTextSize(12f);
                        h.tvStatus.setTypeface(null, android.graphics.Typeface.NORMAL);
                        h.tvStatus.setTextColor(0xBBFFFFFF); break;
                }
            } else {
