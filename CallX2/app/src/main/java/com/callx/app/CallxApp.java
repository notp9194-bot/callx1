...
public class CallxApp extends Application {
    ...
    private static boolean sWentBackground = false;
    private static final Handler sBackgroundHandler = new Handler(Looper.getMainLooper());
    private static Runnable sBackgroundRunnable = null;
    ...
    private void registerForegroundTracking() {
        registerActivityLifecycleCallbacks(new ActivityLifecycleCallbacks() {
            ...
            @Override public void onActivityStarted(Activity a) {
                sActivityRefs++;
                // Cancel any pending background runnable — we're still in foreground
                if (sBackgroundRunnable != null) {
                    sBackgroundHandler.removeCallbacks(sBackgroundRunnable);
            ...
            @Override public void onActivityResumed(Activity a) {
                if (!(a instanceof LockScreenActivity)
                    ...
                    // Only show lock screen if app actually went to background
                    if (sWentBackground) {
                        sWentBackground = false;
            ...
            @Override public void onActivityStopped(Activity a) {
                ...
                if (sActivityRefs == 0) {
                    // Use a 700ms delay — tab switches cause a brief refs==0 moment.
                    // Real backgrounding takes longer; this prevents false lock resets.
                    sBackgroundRunnable = () -> {
                        if (sActivityRefs == 0) {
                            sWentBackground = true;
                            ...
                            // Reset app lock only on true backgrounding
                            LockScreenActivity.resetUnlock();
                        }
                        sBackgroundRunnable = null;
                    };
                    sBackgroundHandler.postDelayed(sBackgroundRunnable, 700);
                }
            }
