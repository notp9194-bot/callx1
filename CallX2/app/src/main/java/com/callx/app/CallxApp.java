...
public class CallxApp extends Application {
    ...
    private void createChannels() {
        ...
        // ── Group call channels ───────────────────────────────────────────────
        // Incoming group call ring channel (highest importance — full screen)
        NotificationChannel gcallIncoming = new NotificationChannel(
            Constants.CHANNEL_GROUP_CALLS_INCOMING, "Incoming Group Calls",
            NotificationManager.IMPORTANCE_HIGH);
        gcallIncoming.setSound(
            RingtoneManager.getDefaultUri(RingtoneManager.TYPE_RINGTONE), attrs);
        gcallIncoming.enableVibration(true);
        gcallIncoming.setVibrationPattern(new long[]{0, 500, 250, 500});
        gcallIncoming.setLockscreenVisibility(android.app.Notification.VISIBILITY_PUBLIC);
        gcallIncoming.setShowBadge(true);
        nm.createNotificationChannel(gcallIncoming);

        // Ongoing group call channel (low importance — persistent, no sound)
        NotificationChannel gcallOngoing = new NotificationChannel(
            Constants.CHANNEL_GROUP_CALLS_ONGOING, "Ongoing Group Call",
            NotificationManager.IMPORTANCE_LOW);
        gcallOngoing.setSound(null, null);
        gcallOngoing.enableVibration(false);
        gcallOngoing.setLockscreenVisibility(android.app.Notification.VISIBILITY_PUBLIC);
        nm.createNotificationChannel(gcallOngoing);

        // Missed group call channel (default importance)
        NotificationChannel gcallMissed = new NotificationChannel(
            Constants.CHANNEL_GROUP_CALLS_MISSED, "Missed Group Calls",
            NotificationManager.IMPORTANCE_DEFAULT);
        gcallMissed.enableVibration(true);
        gcallMissed.setShowBadge(true);
        gcallMissed.setLockscreenVisibility(android.app.Notification.VISIBILITY_PRIVATE);
        nm.createNotificationChannel(gcallMissed);
    }
}
