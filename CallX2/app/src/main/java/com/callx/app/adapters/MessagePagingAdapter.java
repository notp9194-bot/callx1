...
public class MessagePagingAdapter
    ...
    private void bindMessage(@NonNull VH h, @NonNull Message m, int position) {
        ...
        switch (type) {
            ...
            default: // "text", "emoji", etc.
                ...
                if (Boolean.TRUE.equals(m.isEdited)) txt += " (edited)";
        ...
        if (sent && h.ivStatus != null) {
            h.ivStatus.setVisibility(View.VISIBLE);
            String status = m.status != null ? m.status : "sent";
            switch (status) {
                case "seen":
                    h.ivStatus.setImageResource(R.drawable.ic_double_tick_blue);
                    break;
                case "delivered":
                    h.ivStatus.setImageResource(R.drawable.ic_double_tick);
                    break;
                default:
                    h.ivStatus.setImageResource(R.drawable.ic_single_tick);
                    break;
            }
        } else if (h.ivStatus != null) {
    ...
    private void toggleAudio(@NonNull VH h, String url, int position) {
        if (playingPos == position && player != null && player.isPlaying()) {
            player.pause();
            if (h.btnPlayPause != null) h.btnPlayPause.setText("▶");
        } else {
