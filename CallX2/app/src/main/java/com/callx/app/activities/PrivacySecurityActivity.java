...
public class PrivacySecurityActivity extends AppCompatActivity {
    ...
    private BiometricLoginManager bioLoginMgr;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        bioLoginMgr = BiometricLoginManager.getInstance(this);
    ...
    private void setupAppLockSection() {
        ...
        boolean bioHardwareOk = BiometricLoginManager.isHardwareAvailable(this);
        String bioLoginDesc = bioHardwareOk
                ? (bioLoginMgr.isEnabled() ? "Enabled — fingerprint se login hoga" : "Disabled")
                : BiometricLoginManager.getHardwareStatusMessage(this);
        configToggleRow(binding.rowBiometricLogin.getRoot(), R.drawable.ic_shield,
            "Biometric Login",
            bioLoginDesc);
        SwitchCompat swBioLogin = binding.rowBiometricLogin.getRoot().findViewById(R.id.sw_toggle);
        swBioLogin.setChecked(bioLoginMgr.isEnabled());
        swBioLogin.setEnabled(bioHardwareOk);
        swBioLogin.setOnCheckedChangeListener((btn, on) -> {
            if (on) {
                // Enable karne ke liye Firebase user check
                com.google.firebase.auth.FirebaseUser user =
                        com.google.firebase.auth.FirebaseAuth.getInstance().getCurrentUser();
                if (user != null) {
                    bioLoginMgr.enable(user.getUid(), user.getEmail() != null ? user.getEmail() : "");
                    Toast.makeText(this, "Biometric login enable ho gaya!", Toast.LENGTH_SHORT).show();
                } else {
                    swBioLogin.setChecked(false);
                    Toast.makeText(this, "Pehle login karo", Toast.LENGTH_SHORT).show();
                }
            } else {
                bioLoginMgr.disable();
                Toast.makeText(this, "Biometric login disable ho gaya", Toast.LENGTH_SHORT).show();
            }
        });
    }
